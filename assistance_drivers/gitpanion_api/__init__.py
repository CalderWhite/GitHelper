...
import os, ctypes, json, threading
###############################
#        swap words           #
###############################
github_words = {
...
#      general functions      #
###############################
def pypy_run(path,args):
    def start():
        os.chdir("../../mini_pypy")
        os.system('start pypy ' + path + " " + " ".join(args))
    thread = threading.Thread(target=start)
    thread.start()
def check_installed():
    appdata = os.getenv("APPDATA")
    if os.path.exists(appdata + "\\.gitpanion_api"):
        return True
    else:
        return False
def install():
    appdata = os.getenv("APPDATA")
    os.makedirs(appdata + "\\.gitpanion_api")
###############################
#      command functions      #
###############################
...
if __name__ == '__main__':
        #install()
        pypy_run("../assistance_drivers/gitpanion_api/all_files.py",["jarvis"])
